...
def test_get_eicar_file():
    assert_eicar(v.get(io.StringIO(EICAR)))


def test_get_eicar_md5():
    assert_eicar(v.get(EICAR_MD5))


def test_get_eicar_sha1():
    assert_eicar(v.get(EICAR_SHA1))


def test_get_eicar_sha256():
    assert_eicar(v.get(EICAR_SHA256))


def test_scan_eicar():
    r = v.scan(io.StringIO(EICAR))
    r.join()
    assert_eicar(r)


def test_scan_eicar_force():
    r = v.scan(io.StringIO(EICAR), reanalyze=True)
    r.join()
    assert_eicar(r)


def test_get_unknown_report():
    assert v.get(io.StringIO(RAND_STR)) is None


def test_get_unknown_report_hash():
    assert v.get(hashlib.md5(RAND_STR).hexdigest()) is None


def test_entity_too_large():
    size = virustotal.FILE_SIZE_LIMIT + (1 * 1024 * 1024)
    entity_too_large = io.StringIO(size * "a")
    try:
